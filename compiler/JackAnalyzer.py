...
def main(path, log_level, dest_path):
    ...
    if os.path.isfile(path):
        ...
        if dest_path is not None and os.path.isdir(dest_path):
            dest_path = os.path.join(dest_path, str(os.path.basename(path)).replace(".jack", ".vm"))
        elif ".vm" not in dest_path:
            raise ValueError("Destination file path needs to be a XML file")
        elif dest_path is None:
            dest_path = os.path.join(os.path.dirname(path), str(os.path.basename(path)).replace(".jack", ".vm"))
        ...
        for p in files_to_process:
            run(os.path.join(path, p), os.path.join(dest_path, p.replace(".jack", ".vm")))
