...
class CompilationEngine:
  ...
  def write_subroutine_signature(self):
    ...
    if self.subroutine_config["type"] == SubRoutineType.METHOD.value:
      ...
      self.writer.write_pop(SegmentTypes.POINTER.value, 0) # pop pointer 0 -> set THIS
    elif self.subroutine_config["type"] == SubRoutineType.CONSTRUCTOR.value:
      self.writer.write_push(SegmentTypes.CONSTANT.value, self.class_symbol_table.var_count(SegmentTypes.THIS.value)) # push constant numVar
  ...
  def compile_if_statment(self, if_count):
    ...
    self.writer.write_if(f"{self.class_name}.{self.subroutine_config["name"]}.if_{if_count}.pathA") # go if not true
    self.process_rule(TokenType.SYMBOL, ")")
    self.process_rule(TokenType.SYMBOL, "{")
    self.compile_statments()
    self.writer.write_goto(f"{self.class_name}.{self.subroutine_config["name"]}.if_{if_count}.pathB") # skip statements for if not true
    self.process_rule(TokenType.SYMBOL, "}")
    there_is_else = False
    if self.tokenizer.peek() == "else":
      ...
      self.process_rule(TokenType.SYMBOL, "{")
    self.writer.write_label(f"{self.class_name}.{self.subroutine_config["name"]}.if_{if_count}.pathA")
    if there_is_else: # there has to be a more elegant way of doing this. Line above MUST be printed regardless. Lines below cannot be printed if no ELSE
      self.compile_statments()
      self.process_rule(TokenType.SYMBOL, "}")
    self.writer.write_label(f"{self.class_name}.{self.subroutine_config["name"]}.if_{if_count}.pathB")
    logger.debug("</ifStatement>")
  ...
  def compile_while_statment(self, while_count):
    ...
    self.writer.write_label(f"{self.class_name}.{self.subroutine_config["name"]}.while_{while_count}.pathA") # top
    self.process_rule(TokenType.KEYWORD, "while")
    self.process_rule(TokenType.SYMBOL, "(")
    self.compile_expression()
    self.writer.write_arithmetic("not")
    self.writer.write_if(f"{self.class_name}.{self.subroutine_config["name"]}.while_{while_count}.pathB") # break
    self.process_rule(TokenType.SYMBOL, ")")
    self.process_rule(TokenType.SYMBOL, "{")
    self.compile_statments()
    self.writer.write_goto(f"{self.class_name}.{self.subroutine_config["name"]}.while_{while_count}.pathA") # back to top
    self.process_rule(TokenType.SYMBOL, "}")
    self.writer.write_label(f"{self.class_name}.{self.subroutine_config["name"]}.while_{while_count}.pathB") # break label
    logger.debug("</whileStatement>")
  ...
  def compile_class_var_dec(self):
    self.class_var_config["kind"] = SegmentTypes.THIS.value if self.process_rule(TokenType.KEYWORD, "static", "field") == "field" else SegmentTypes.STATIC.value# static | field
    ...
    logger.debug(f"Class Variable defined - var_name: {self.class_var_config["name"]}, var_type: {self.class_var_config["type"]}")
    while self.tokenizer.peek() == ",":
      self.process_rule(TokenType.SYMBOL, ",")
      self.class_var_config["name"] = self.process_token(TokenType.IDENTIFIER) # variable name
      self.class_symbol_table.define(self.class_var_config["name"], self.class_var_config["type"], self.class_var_config["kind"])
  ...
  def resolve_class_type_to_call(self, name):
    ...
    elif name in self.class_symbol_table.table:
